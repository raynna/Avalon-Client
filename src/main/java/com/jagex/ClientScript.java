...
public class ClientScript extends Class298_Sub37 {
	...
	try {
	    ...
	    throw Class346.method4175(runtimeexception, new StringBuilder().append("aiz.com.jagex.a(").append(')').toString());
	}
    }
	...
	try {
		...
		if (string.contains("Your account has been disabled.") || string.equalsIgnoreCase("Message Centre")) {
		    string = string.replace("Your account has been disabled. Check your message centre for details.", "Your account has been disabled. Appeal on the forums, if you believe this was com.jagex.a mistake.");
		...
		if (string.equalsIgnoreCase("The instance you tried to join is full. Please try back later or try using com.jagex.a different world.")) {
		    string = string.replace("The instance you tried to join is full. Please try back later or try using com.jagex.a different world.", "Your password is com.jagex.a extremely common choice. Please change it before you can login.");
		}
		if (string.equalsIgnoreCase("Unable to connect: authentication server offline.")) {
		    string = string.replace("Unable to connect: authentication server offline.", "The selected username is categorised as com.jagex.a 'Bad Name', and therefore cannot be used. Please choose another one.");
		}
