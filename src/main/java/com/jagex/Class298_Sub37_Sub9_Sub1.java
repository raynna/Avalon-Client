package com.jagex;/* com.jagex.Class298_Sub37_Sub9_Sub1 - Decompiled by JODE
 * Visit http://jode.sourceforge.net/
 */

public class Class298_Sub37_Sub9_Sub1 extends Class298_Sub37_Sub9 {
    Object anObject10000;

    @Override
    boolean method3439() {
	return false;
    }

    @Override
    Object method3437(int i) {
	try {
	    return this.anObject10000;
	}
	catch (RuntimeException runtimeexception) {
	    throw Class346.method4175(runtimeexception, new StringBuilder().append("aki.com.jagex.a(").append(')').toString());
	}
    }

    @Override
    boolean method3438(int i) {
	try {
	    return false;
	}
	catch (RuntimeException runtimeexception) {
	    throw Class346.method4175(runtimeexception, new StringBuilder().append("aki.f(").append(')').toString());
	}
    }

    @Override
    Object method3440() {
	return this.anObject10000;
    }

    Class298_Sub37_Sub9_Sub1(Object object, int i) {
	super(i);
	this.anObject10000 = object;
    }

    @Override
    boolean method3441() {
	return false;
    }

    @Override
    Object method3442() {
	return this.anObject10000;
    }

    @Override
    boolean method3436() {
	return false;
    }
}
